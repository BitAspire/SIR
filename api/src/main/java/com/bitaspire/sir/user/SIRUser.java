...
import com.bitaspire.sir.SoundUtils;
import org.bukkit.*;
...
public interface SIRUser {
    ...
    default void teleport(World world, double x, double y, double z, float... directions) {
        if (!isOnline() || world == null) return;

        Location location = new Location(world, x, y, z);
        if (directions != null)
            switch (directions.length) {
                case 1:
                    location.setYaw(directions[0]);
                    break;

                case 2 :
                    location.setYaw(directions[0]);
                    location.setPitch(directions[1]);
                    break;
    ...
    default void teleport(String... arguments) {
        ...
        if (arguments.length > 1 && arguments.length < 5) {
            try {
                location.setX(Double.parseDouble(arguments[1]));
            } catch (Exception ignored) {}

            if (arguments.length > 2)
                try {
                    location.setY(Double.parseDouble(arguments[2]));
                } catch (Exception ignored) {}

            if (arguments.length > 3)
                try {
                    location.setZ(Double.parseDouble(arguments[3]));
                } catch (Exception ignored) {}
        }

        if (arguments.length > 4)
            try {
                location.setYaw(Float.parseFloat(arguments[4]));
            } catch (Exception ignored) {}

        if (arguments.length > 5)
            try {
                location.setPitch(Float.parseFloat(arguments[5]));
            } catch (Exception ignored) {}
