...
final class Command extends SIRCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, String[] args) {
        ...
        if (category == null) return getArgumentCheck().test(sender, args[0]);

        if (args.length < 3) {
            String path = category == SettingsProvider.Category.MODULES ? "help.modules" : "help.commands";
            return Utils.create(this, sender).send(path);
        }

        ChatToggleState state = ChatToggleState.fromInput(args[2]);
        if (state == null)
            return getArgumentCheck().test(sender, args[2]);

        return handleToggle(sender, user, category, args[1], state);
    }
                                 ...
                                 ChatToggleState state)
    {
        ...
        if (toggleable == null)
            return Utils.create(this, sender)
                    ...
                    .addPlaceholder("{name}", name).send("toggle.not-found");
