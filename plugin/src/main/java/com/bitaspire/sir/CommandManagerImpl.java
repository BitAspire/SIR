...
final class CommandManagerImpl implements CommandManager {
    ...
    public void load(File jarFile, boolean syncCommands) {
        ...
        try {
            ...
            if (!enabled) {
            if (provider instanceof StandaloneProvider) ((StandaloneProvider) provider).setRegistered(false);
                log(LogLevel.INFO, "Command provider '" + file.getName() + "' is disabled, skipping registration.");
                return;
            }
            ...
            if (provider instanceof StandaloneProvider)
                ((StandaloneProvider) provider).setRegistered(true);
    ...
    public Boolean getCommandOverride(String providerName, String commandKey) {
        ...
        ProviderState state = ensureProviderState(loaded.information.getName());
        return state.overrides.get(key(commandKey));
    }
